...
class LoginFragment : Fragment() {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        binding.gameModeGroup.setOnCheckedChangeListener { _, checkedId ->
            when (checkedId) {
                R.id.singlePlayerRadioButton -> {
                    binding.editname1.hint = "enter you name"
                    binding.editname2.visibility = View.GONE
                }
                R.id.multiPlayerRadioButton -> {
                    binding.editname1.hint = "enter first player"
                    binding.editname2.visibility = View.VISIBLE
                }
                R.id.aiPlayerRadioButton -> {
                    binding.editname1.hint = "enter your name"
                    binding.editname2.visibility = View.GONE
                }
            }
        }
        ...
        binding.buttongo.setOnClickListener {
            ...
            if (selectedModeId == -1) {
                Toast.makeText(context, "Please select game mode.", Toast.LENGTH_SHORT).show()
            } else {
